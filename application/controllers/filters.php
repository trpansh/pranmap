	...
	class Filters extends CI_Controller {
		...
		function index() {
			if ($this->input->post()) {
				...
				if ($batch == FALSE && $tool == FALSE && $title == FALSE && $district == FALSE && $sector == FALSE && $theme == FALSE && $funding == FALSE && $status == FALSE) {
					$data['error'] = 'No filter selected.';
				} else {
					$data = $this->search_m->filter($batch, $tool, $title, $district, $sector, $theme, $funding, $status);
